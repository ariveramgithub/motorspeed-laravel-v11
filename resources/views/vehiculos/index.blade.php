...
<div class="container">
    ...
    <div class="row">
    <div class="col-md-1">
        <form method="POST" id="deleteForm" action="{{ route('vehiculos.destroy', 0)  }}">
        @csrf
        @method('DELETE')
        ...
        <div class="col-md-1">
            <div class="input-group">
                <a href="{{ url('/vehiculos/create') }}" class="btn btn-primary" aria-label="Add" title="Agregar">
        ...
        <table class="table table-striped table-hover table-borderless">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><input type="checkbox" name="deleteId[]" value="{{ $vehiculo->id }}" aria-label="Select ID {{ $vehiculo->id }}"></td>
                    <td>{{ $vehiculo->patente }}</td>
                    <td>{{ $vehiculo->marca }}</td>
                    <td>{{ $vehiculo->modelo }}</td>
                    <td>{{ $vehiculo->color }}</td>
                    <td>{{ $vehiculo->year }}</td>
                    <td>{{ ($vehiculo->kilometraje) > 0 ? number_format($vehiculo->kilometraje, 0, ',', '.') : 0 }}</td>
                    <td>{{ $enumLabels[$vehiculo->transmision] }}</td>
                    <td>{{ $enumLabels[$vehiculo->combustible] }}</td>
                    @if ($vehiculo->cliente_nombre)
                    <td><a href="{{ url('/clientes').'/'.$vehiculo->cliente_id.'/edit' }}">{{ $vehiculo->cliente_nombre}}</a></td>
                    @else
    ...
    <div class="row">
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                ...
                ])><a class="page-link" href="/vehiculos?page={{ $i }}{{ ($querystring != '') ? '&query=' . $querystring : '' }}">{{ $i }}</a></li>
                @endfor
