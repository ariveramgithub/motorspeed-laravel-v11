...
<div class="container">
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header fs-4 fw-semibold">{{ __('Modificar órden de trabajo') }}</div>
                <div class="card-body">
                    <form method="POST" action="{{ route('ordentrabajos.update', $orden->id) }}">
                    @csrf
                    @method('PUT')
                    <input type="hidden" id="servicios" name="servicios" value="{{ $orden->servicios }}"> 
                    <p class="fw-bold">Datos cliente y vehículo</p>
                    <div class="input-group">
                        <div class="form-floating mb-3 col-md-6">
                            <input name="vehiculo_id" value="{{ ( old('vehiculo_id') ) ? old('vehiculo_id') : $orden->vehiculo_id }}" class="form-control @error('vehiculo_id') is-invalid @enderror" list="vehiculoslistOptions" id="inputVehiculoId" placeholder="Type to search..." required >
                        ...
                        <div class="form-floating mb-3 col-md-6">
                            <input name="cliente_id" value="{{ ( old('cliente_id') ) ? old('cliente_id') : $orden->cliente_id }}" class="form-control @error('cliente_id') is-invalid @enderror" list="clienteslistOptions" id="inputClienteId" placeholder="Type to search..." required >
                    ...
                    <div id="serviciosList" class="d-flex gap-2 justify-content-center pb-3">
                    @if ($orden->servicios)
                    @foreach (explode("|", $orden->servicios) as $servicio)
                        <span class="badge rounded-pill text-bg-primary p-2">
                            <span>{{ $servicio }}</span>
                            <a data-id="{{ $servicio }}" class="deleteServicio" href="#">
                    ...
                    <div class="input-group">
                        <div class="form-floating mb-3 col-md-4">
                            <input name="inicio" value="{{ $orden->inicio }}" type="datetime-local" class="form-control @error('inicio') is-invalid @enderror" id="inputInicio" placeholder="" required >
                            @error('inicio')
                        ...
                        <div class="form-floating mb-3 col-md-4">
                            <input name="termino" value="{{ $orden->termino }}" type="datetime-local" class="form-control @error('termino') is-invalid @enderror" id="inputTermino" placeholder="" required >
                            @error('termino')
                        ...
                        <div class="form-floating mb-3 col-md-4">
                            <input name="valor" value="{{ $orden->valor }}" type="number" class="form-control @error('valor') is-invalid @enderror" id="inputValor" placeholder="" required >
                            @error('valor')
                    ...
                    <div class="form-floating mb-3">
                        <textarea name="detalle_cliente" class="form-control @error('detalle_cliente') is-invalid @enderror" placeholder="" id="inputDetalleCliente" style="height: 200px">{{ $orden->detalle_cliente }}</textarea>
                    ...
                    <div class="form-floating mb-3">
                        <textarea name="detalle_taller" class="form-control @error('detalle_taller') is-invalid @enderror" placeholder="" id="inputDetalleTaller" style="height: 200px">{{ $orden->detalle_taller }}</textarea>
                    ...
                    <div class="mb-3 d-flex justify-content-center">
                        ...
                        <input type="radio" class="btn-check" name="estado" value="{{ $estado->estado }}" id="estadoOption{{ $estado->id }}" autocomplete="off" @checked($estado->estado == $orden->estado)>
...
<form id="deleteForm" method="POST" action="{{ route('ordentrabajos.destroy', $orden->id) }}">
@csrf
@method('DELETE')
</form>
